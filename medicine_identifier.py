...
def identify_medicine_info(text):
    expiry = re.search(r'expiry\s*[:\-]?\s*(\w+\s*\d{4})', text, re.IGNORECASE)
    name = re.search(r'(paracetamol|ibuprofen|amoxicillin|dolo|crocin|cetirizine)', text, re.IGNORECASE)
    use_case = {
